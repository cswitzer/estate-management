...
# Not to be confused with a view, since this is a model. This is just a nice, generic way
# to keep track of user views on any model in the project.
class ContentView(TimeStampedModel):
    ...
    # Content type is a reference to ANY model in the project
    ...
    def record_view(cls, content_object, user: User, viewer_ip: str) -> None:
        ...
        try:
            view, created = cls.objects.get_or_create(
                ...
                default={"user": user, "viewer_ip": viewer_ip},
            )
            if not created:
                pass
